...
class MediaCheckerGUI:
    ...
    def update_file_dates(self):
        ...
        for path, date, date_type, _ in self.check_results['with_date']:
            try:
                # 解析日期字符串
                date_obj = datetime.strptime(date, '%Y-%m-%d %H:%M:%S')
                files_to_update.append((path, date_obj))
            except ValueError:
    ...
    def _update_dates_thread(self, files_to_update):
        ...
        for path, date_obj in files_to_update:
            try:
                # 转换为Windows时间格式
                ...
                success_count += 1
                print(f"成功修改文件日期: {path} -> {date_obj}")
        ...
        # 在主线程中显示结果
        self.root.after(0, lambda: self._show_update_result(success_count, fail_count))
        
    def _show_update_result(self, success_count, fail_count):
        """显示修改结果"""
        messagebox.showinfo("完成", f"文件日期修改完成\n成功: {success_count} 个文件\n失败: {fail_count} 个文件")
